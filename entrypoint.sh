...
MONIKER=d1523b1c-85a1-40fb-8b55-6bf6d9ae0a0a &&
    ...
    sudo --preserve-env docker inspect --format "{{.NetworkSettings.Networks.bridge.IPAddress}}" $(cat registry) | sudo --preserve-env docker exec --interactive $(cat docker) tee -a /etc/hosts &&
    (cat > ${TFILE} <<EOF
{
